...
source_amp      = 114.008e3;      % source pressure [Pa]
...
alpha_coeff_muscle    = 0.62;
alpha_coeff_brain     = 0.59;
...
brain_mask = logical(flip(flip(permute(niftiread('../Brain_mask.nii.gz'),[2 3 1]),2),3));
...
medium.sound_speed = c_water + skin_mask*(c_skin-c_water) + muscle_mask*(c_muscle-c_water) + brain_mask*(c_brain-c_water) + skull_mask.*skull_porosity*(c_skull_min-c_water) + skull_mask.*(1-skull_porosity)*(c_skull_max-c_water);
medium.density = rho_water + skin_mask*(rho_skin-rho_water) + muscle_mask*(rho_muscle-rho_water) + brain_mask*(rho_brain-rho_water) + skull_mask.*skull_porosity*(rho_skull_min-rho_water) + skull_mask.*(1-skull_porosity)*(rho_skull_max-rho_water);
...
alpha_coeff_true =  alpha_coeff_water ...
                  ...
                  + brain_mask*(alpha_coeff_brain-alpha_coeff_water);
...
s = sensor.record_start_index;
save('start_index.txt','s','-ascii');
clear source sensor;
save('pre.mat','-v7.3');
CheckPoint = fopen('CheckPoint_Part1.txt','w');
fprintf(CheckPoint,"%s","Part 1: Successfull");
fclose(CheckPoint);
quit;